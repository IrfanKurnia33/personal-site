// Music entries for the /music page. Add releases, playlists, or favorites.
// `embeds` holds one or more platform *embed* URLs (Spotify/YouTube/SoundCloud/Apple).
// Leave `embeds` empty to render a simple link card instead of iframes.

export interface MusicItem {
  title: string;
  kind: "Single" | "EP" | "Live" | "Acoustic" | "Demo";
  /** One or more embed URLs (iframe src). Optional. */
  embeds?: string[];
  /** External link (shown as a button and used when no embed). */
  href?: string;
  note?: string;
}
